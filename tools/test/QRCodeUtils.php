...
class QRCodeUtils
{
    ...
    /**
     * @param $codePatch string 工作目录下的路径
     * @param $qrCodeName string 二维码名称
     * @param $content string 二维码内容
     * @param bool $forceFlush 是否强制覆盖
     * @param string $level
     * @param int $size
     * @param int $margin
     * @return string
     * @throws \Exception
     */
    public static function qrCode($codePatch, $qrCodeName, $content, $forceFlush = false, $level = 'L', $size = 9, $margin = 1)
    {
        $codePatchTmp = $_SERVER['DOCUMENT_ROOT'] . '/' . ltrim($codePatch, '/');
        return self::createQRCode($qrCodeName, $content, $codePatchTmp, $forceFlush, $level, $size, $margin);
    }
...
//        $codePatchTmp=$patch.$codePatch;
        ...
        try {
            ...
            throw new \Exception("二维码生成失败，错误信息为：" . $e->getMessage(), 500);
        }
    }
